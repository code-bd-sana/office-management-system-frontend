...
interface AttendanceModalRowProps {
...
export function AttendanceModalRow({ record }: AttendanceModalRowProps) {
  ...
  return (
    <TableRow className="border-b border-border/40 hover:bg-muted/30">
      ...
      <TableCell className="whitespace-nowrap py-3.5">
        <span
          className={`inline-block rounded-sm px-2.5 py-1 text-xs font-semibold w-25 text-center ${statusStyle.bg} ${statusStyle.text}`}
        >
          {statusStyle.label}
        </span>
      </TableCell>
