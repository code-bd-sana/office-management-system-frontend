...
interface ModalTablePaginationProps {
  currentPage: number;
  totalPages: number;
  rowsPerPage: number;
  totalRecords: number;
...
export function ModalTablePagination({
  ...
  onPageChange,
  onRowsPerPageChange,
}: ModalTablePaginationProps) {
  return (
    <div className="flex items-center justify-between rounded-b-sm border border-t-0 bg-[#E8EAEE] px-4 py-2.5 backdrop-blur-sm sm:px-5 sm:py-3">
      {/* Left: record range */}
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="flex h-7 w-7 items-center justify-center rounded-sm border border-border/60 text-foreground/60 transition-colors hover:bg-muted disabled:cursor-not-allowed disabled:opacity-30"
