...
/* ── Helpers ────────────────────────────────────────────────── */
function resolveName(field: { _id: string; name: string } | string | null | undefined): string {
  if (!field) return '—';
  ...
  return (
    <TableRow className='border-b border-border/40 hover:bg-muted/30'>
      ...
      <TableCell className='py-3.5 text-sm font-semibold text-foreground'>
        ...
        {project.orderId && (
          <div className='text-xs font-normal text-muted-foreground'>{project.orderId}</div>
        )}
      </TableCell>
