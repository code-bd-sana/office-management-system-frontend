...
interface LearningModalRowProps {
  record: LearningRecord;
}

export function LearningModalRow({ record }: LearningModalRowProps) {
  return (
    <TableRow className="border-b border-border/40 hover:bg-muted/30">
      <TableCell className="py-3.5 pl-5">
        <input
          ...
          aria-label={`Select record ${record.rowNumber}`}
        />
      </TableCell>
      ...
      <TableCell className="py-3.5">
        <Link
          href="#"
          className="inline-block rounded-sm border border-brand-navy bg-brand-navy px-4 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-brand-navy/90"
        >
          View Details
        </Link>
      </TableCell>
